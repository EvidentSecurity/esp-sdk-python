...
class RegionSuppressionsApi(object):
    """
    ...
    def create_region_suppression(self, regions, external_account_ids, reason, resource, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_region_suppression(regions, external_account_ids, reason, resource, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.create_region_suppression_with_http_info(regions, external_account_ids, reason, resource, **kwargs)
        else:
            (data) = self.create_region_suppression_with_http_info(regions, external_account_ids, reason, resource, **kwargs)
            return data

    def create_region_suppression_with_http_info(self, regions, external_account_ids, reason, resource, **kwargs):
        """
        A successful call to this API creates a new region suppression for the supplied regions . The body of the request must contain a json api compliant hash of attributes with type suppression/regions.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_region_suppression_with_http_info(regions, external_account_ids, reason, resource, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method create_region_suppression" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'regions' is set
        if ('regions' not in params) or (params['regions'] is None):
            raise ValueError("Missing the required parameter `regions` when calling `create_region_suppression`")
        ...
        if ('external_account_ids' not in params) or (params['external_account_ids'] is None):
            raise ValueError("Missing the required parameter `external_account_ids` when calling `create_region_suppression`")
        # verify the required parameter 'reason' is set
        if ('reason' not in params) or (params['reason'] is None):
            raise ValueError("Missing the required parameter `reason` when calling `create_region_suppression`")
        # verify the required parameter 'resource' is set
        if ('resource' not in params) or (params['resource'] is None):
            raise ValueError("Missing the required parameter `resource` when calling `create_region_suppression`")


        collection_formats = {}

        resource_path = '/api/v2/suppressions/regions.json_api'.replace('{format}', 'json')
        path_params = {}
